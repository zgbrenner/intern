...
The read-only QA workflow may create a blocked manifest with `rendered_fidelity: pending`; `--allow-pending` exists only so the complete QA artifact can be uploaded for inspection. The tagged release workflow never uses that flag. It reruns Q4 and Q8 through the production evaluator, recaptures the UI, builds and installs the exact installer, creates a new run-bound manifest, validates it without exceptions, and only then reaches the publishing step.
