...
| Q4 production evaluation | pending | Every signed gold fixture through packaged worker, production packet/prompt/client/validator, llama.cpp b10361, exact Q4+F16 projector |
| Q8 production evaluation | pending | Same corpus and production path with exact Q8+F16 projector |
| Model acceptance | pending | Production parser replay and `validate_proposal` replay; 100% valid eligible responses; every ambiguous result in Needs Review; zero unsupported Ready dates/parties; Q4 accuracy within 2 points of Q8 or exact Q8 manifest fallback |
...
`.github/workflows/qa.yml` is manual, read-only, and cannot create a tag, push a commit, or publish a release. It runs the browser/Rust/native/runtime/installer gates, generates the screenshot and Q4/Q8 report, and uploads the checklist, fidelity ledger, evaluation, screenshot, diagnostics, installer, installed-app smoke report, and hash manifest as QA artifacts. Its model step uses `src-tauri/src/bin/intern-model-evaluator.rs`, which calls the production worker adapter, document-packet builder, model client/prompt, and deterministic validator. Before acceptance, `intern-model-evidence-validator` re-extracts every signed fixture and replays each recorded raw proposal through the current production `validate_proposal`.

The release workflow does not trust a checked-in completed report or a claimed run ID. The exact annotated-tag run rebuilds the evaluator and runtime, reruns Q4 and Q8 over every signed fixture, replays the new report through production extraction and validation, recaptures the UI, installs and launches the produced application, and creates a new manifest bound to the tag commit and workflow run. Publishing is unreachable unless `validate-release-evidence.mjs` accepts all artifact hashes plus model, rendered-fidelity, and installed-core sign-offs. The checked-in fidelity sign-off remains pending, so release is intentionally blocked until the hosted screenshot is inspected.
