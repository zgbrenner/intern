...
**Release status: blocked pending fresh rendered-fidelity review and the dispatched release run.** Alpha.3 changes non-QA release inputs, so the existing alpha.2 capture/sign-off cannot be accepted for this release. The primary must produce and review a new capture bound to the final non-QA `release_inputs_sha256`; until then rendered fidelity is pending/blocked. Do not edit the current screenshot or sign-off to simulate acceptance.
...
| Rendered fidelity review | pending/blocked | A fresh capture and reviewer sign-off bound to the final alpha.3 non-QA release-input digest. The checked-in alpha.2 sign-off is historical evidence only and the workflow must reject it as stale. |
...
`Cargo.lock` is committed and every hosted Rust command uses `--locked`. No release tag or publish action is authorized until a fresh alpha.3 fidelity sign-off plus the dispatched run's own model evaluation and installed-core evidence are accepted. The release workflow cryptographically verifies the updater signature offline, generates `SHA256SUMS.txt` before evidence acceptance, and binds every SPDX SBOM, runtime manifest, notices, updater manifest, installer, and required log in the evidence manifest. `SHA256SUMS.txt` deliberately excludes itself and `release-evidence-manifest.json` to avoid a digest cycle; the evidence manifest hashes the checksum file instead.
