...
`.github/workflows/qa.yml` is manual, read-only, and cannot create a tag, push a commit, or publish a release. It runs the browser/Rust/native/runtime/installer gates, generates the screenshot and Q4/Q8 report, and uploads the checklist, fidelity ledger, evaluation, screenshot, diagnostics, and installer as QA artifacts. Its model step uses `src-tauri/src/bin/intern-model-evaluator.rs`, which calls the production worker adapter, document-packet builder, model client/prompt, and deterministic validator. Before acceptance, `intern-model-evidence-validator` re-extracts every signed fixture and replays each recorded raw proposal through the current production `validate_proposal`; the release workflow repeats that replay against its staged worker.

The release workflow still calls `scripts/validate-model-evaluation.mjs` without `--allow-pending`. Therefore this branch cannot publish while `docs/qa/model-evaluation.json` is pending. A completed report is accepted only from Windows evidence with exact signed per-fixture proposals, derived field results and unsupported facts, timings, and peak RSS.

`Cargo.lock` is absent from this worktree baseline, so all locked Rust/Windows workflows remain blocked until the reviewed integration branch supplies the committed lockfile. No release tag or publish action is authorized by this checklist.
