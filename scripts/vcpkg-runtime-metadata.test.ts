...
it('derives DLL owners and exact versions from vcpkg status and installed lists', async () => {
  ...
  // Real vcpkg .list files also enumerate directories (trailing slash) that
  // are shared between packages; ownership applies to files only.
  await writeFile(
    join(root, 'vcpkg/info/tesseract_5.5.2_x64-windows.list'),
    'x64-windows/\nx64-windows/tools/\nx64-windows/tools/tesseract/\nx64-windows/tools/tesseract/tesseract.exe\n',
  );
  await writeFile(
    join(root, 'vcpkg/info/leptonica_1.85.0#1_x64-windows.list'),
    'x64-windows/\nx64-windows/bin/\nx64-windows/bin/leptonica-6.dll\n',
  );
  await writeFile(
    join(root, 'vcpkg/info/libpng_1.6.50_x64-windows.list'),
  ...
  expect(Object.keys(metadata.owners).every((path) => !path.endsWith('/'))).toBe(true);
});
