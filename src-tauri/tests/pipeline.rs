...
use intern_core::{
    DateKind, Evidence, ErrorCode, ExtractedDocument, ModelProposal, OperationReceipt,
...
impl PipelineEventSink for RecordingEvents {
    fn queue_changed(&self) { self.changed.fetch_add(1, Ordering::SeqCst); }
...
impl ModelBoundary for FakeModel {
    fn propose(&self, _document: &intern_app::model::client::DocumentInput) -> Result<ModelProposal, ModelFailure> {
        ...
        self.responses.lock().unwrap().pop_front().unwrap_or_else(|| {
            Err(ModelFailure::fatal("MODEL_RESPONSE_INVALID"))
        })
    }
}
...
impl ModelBoundary for BlockingModel {
    ...
    fn cancel(&self) -> Result<(), ModelFailure> {
        ...
        if self.cancel_succeeds { self.release_request(); }
        let (lock, wake) = &self.cancel_release;
        let mut released = lock.lock().unwrap();
        while !*released { released = wake.wait(released).unwrap(); }
        if self.cancel_succeeds { Ok(()) } else { Err(ModelFailure::fatal("MODEL_CANCEL_FAILED")) }
    }
}
...
impl FakeFiles {
    fn trust(&self, path: &Path, hash: &str) {
        self.hashes.lock().unwrap().insert(path.to_path_buf(), hash.to_owned());
    }

    fn forget(&self, path: &Path) { self.hashes.lock().unwrap().remove(path); }
    fn fail_next_apply(&self) { self.fail_next_apply.store(1, Ordering::SeqCst); }
}

impl FileActions for FakeFiles {
    fn fingerprint(&self, path: &Path) -> Result<String, PipelineError> {
        self.hashes.lock().unwrap().get(path).cloned().ok_or_else(|| {
            PipelineError::new("FILE_MISSING", "source file is unavailable")
        })
    }

    fn apply(&self, item: &QueueItem, _destination: &Path) -> Result<(), PipelineError> {
        if self.fail_next_apply.fetch_update(Ordering::SeqCst, Ordering::SeqCst, |remaining| {
...
fn proposal(confidence: f32, needs_review: bool) -> ModelProposal {
    ModelProposal {
        ...
        confidence,
        needs_review,
        review_reasons: if needs_review { vec!["uncertain".into()] } else { vec![] },
        evidence: Evidence {
...
fn mixed_queue_is_sequential_and_ready_review_are_evidence_gated() {
    ...
    let worker = Arc::new(FakeWorker::new(vec![
        Ok(parsed("Employment Agreement signed April 12, 2024 by John Smith and Acme Corporation.")),
        Ok(parsed("Employment Agreement mentioning Acme Corporation only.")),
    ]));
    let model = Arc::new(FakeModel::new(vec![Ok(proposal(0.94, false)), Ok(proposal(0.94, false))]));
    let files = Arc::new(FakeFiles::default());
    files.trust(&first, "first-hash");
    files.trust(&second, "second-hash");
    let pipeline = pipeline(temp.path(), Arc::clone(&worker), model, Arc::clone(&files), AppSettings::default());
    ...
    assert_eq!(items[0].proposal.as_ref().unwrap().status, ProposalStatus::Ready);
    assert_eq!(items[1].proposal.as_ref().unwrap().status, ProposalStatus::NeedsReview);
}
...
fn retryable_malformed_model_output_retries_once_only() {
    ...
    let pipeline = pipeline(temp.path(), worker, Arc::clone(&model), files, AppSettings::default());
...
fn failed_item_does_not_block_the_next_and_worker_restarts_only_once() {
    ...
    let pipeline = pipeline(temp.path(), Arc::clone(&worker), model, files, AppSettings::default());
...
fn pause_starts_no_item_and_cancel_interrupts_the_active_worker_request() {
    ...
    let pipeline = Arc::new(pipeline(temp.path(), Arc::clone(&worker), model, files, AppSettings::default()));

    pipeline.enqueue_files(&[paused_path, cancel_path]).unwrap();
    pipeline.pause();
    pipeline.run_until_idle().unwrap();
    assert!(pipeline.list().unwrap().iter().all(|item| item.status == QueueStatus::Queued));

    pipeline.resume();
    let running = Arc::clone(&pipeline);
    let join = thread::spawn(move || running.run_next());
    while worker.active.load(Ordering::SeqCst) == 0 { thread::yield_now(); }
...
fn automatic_apply_targets_only_ready_items_and_rechecks_source_fingerprint() {
    ...
    let pipeline = pipeline(
        ...
        model,
        Arc::clone(&files),
        AppSettings { automatic_rename: true, ..AppSettings::default() },
    );
    pipeline.enqueue_files(&[ready.clone(), review, changed.clone()]).unwrap();
    ...
    assert!(items[2].proposal.as_ref().unwrap().reasons.iter().any(|reason| reason == "FILE_CHANGED"));
}
...
fn fingerprint_failure_is_durable_and_does_not_stop_the_queue() {
    ...
    files.trust(&second, "second-hash");
    let pipeline = pipeline(
        temp.path(), worker, model, Arc::clone(&files),
        AppSettings { automatic_rename: true, ..AppSettings::default() },
    );
    ...
    assert!(items[0].proposal.as_ref().unwrap().reasons.iter().any(|reason| reason == "FILE_MISSING"));
...
fn apply_failure_is_durable_and_does_not_stop_the_queue() {
    ...
    let items = pipeline.list().unwrap();
    assert_eq!(items[0].status, QueueStatus::NeedsReview);
    assert!(items[0].proposal.as_ref().unwrap().reasons.iter().any(|reason| reason == "MOVE_FAILED"));
    assert_eq!(files.applies.lock().unwrap().as_slice(), &[items[1].id]);
}
...
fn settings_failure_is_recorded_for_each_item_while_the_queue_drains() {
    ...
    let items = pipeline.list().unwrap();
    assert!(items.iter().all(|item| item.status == QueueStatus::NeedsReview));
    assert!(items.iter().all(|item| item.proposal.as_ref().unwrap().reasons.iter()
...
fn field_affecting_parser_warning_prevents_ready() {
    ...
    let mut document = parsed("Employment Agreement signed April 12, 2024 by John Smith and Acme Corporation.");
...
fn initial_proposal_and_ready_transition_commit_atomically() {
    ...
    let proposals: i64 = connection.query_row("SELECT COUNT(*) FROM proposals", [], |row| row.get(0)).unwrap();
...
fn real_sqlite_and_core_file_actions_apply_then_undo_the_operation_receipt() {
    ...
    settings.save(&AppSettings { automatic_rename: true, ..AppSettings::default() }).unwrap();
...
fn model_timeout_does_not_start_another_proposal_until_cancel_is_terminal() {
    ...
    let pipeline = Arc::new(
        Pipeline::open(
            temp.path().join("queue.sqlite3"), worker, model.clone(), files,
            Arc::new(RecordingEvents::default()), SettingsStore::new(temp.path().join("settings.json")),
        ).unwrap().with_model_timeout(Duration::from_millis(20)),
    );
    pipeline.enqueue_files(&[first, second]).unwrap();
    let running = Arc::clone(&pipeline);
...
fn lost_processing_lease_cancels_worker_and_pauses_before_more_work() {
    ...
    let pipeline = Arc::new(
        pipeline(temp.path(), Arc::clone(&worker), model, files, AppSettings::default())
    ...
    let join = thread::spawn(move || running.run_next());
    while worker.active.load(Ordering::SeqCst) == 0 { thread::yield_now(); }
    Connection::open(temp.path().join("queue.sqlite3")).unwrap()
        .execute("DELETE FROM queue_sessions", []).unwrap();
