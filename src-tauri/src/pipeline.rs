...
use intern_core::{
    ErrorCode, ExtractedDocument, FileApplier, InternError, ModelProposal, OperationReceipt,
    ProposalStatus, QueueItem, QueueStatus, QueueStore, ReviewReason, StdFileSystem,
    ValidationOutcome, build_document_packet, compose_filename, validate_proposal,
};
...
impl Pipeline {
    ...
    fn run_next_inner(&self) -> PipelineResult<bool> {
        ...
        let parsed = match self
            .worker
            .parse(&request_id, &item.source_path, &mut forward_progress)
        {
            Ok(parsed) => parsed,
            Err(error) => {
                self.active_item.store(0, Ordering::SeqCst);
