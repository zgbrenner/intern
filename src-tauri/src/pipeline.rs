...
use intern_core::{
    ErrorCode, ExtractedDocument, FileApplier, InternError, ModelProposal,
    OperationReceipt, ProposalStatus, QueueItem, QueueStatus, QueueStore, ReviewReason,
    StdFileSystem, ValidationOutcome, build_document_packet, compose_filename, validate_proposal,
};
use rusqlite::{Connection, OptionalExtension, params};
use serde::{Deserialize, Serialize};

use crate::{
    model::client::{DocumentInput, ImageInput, ModelClient},
...
impl LeaseKeeper {
    pub fn start(store: Arc<QueueStore>, item_id: i64, interval: std::time::Duration) -> PipelineResult<Self> {
    ...
    fn start_with_cancel(
        ...
        let join = std::thread::Builder::new().name(format!("intern-lease-{item_id}")).spawn(move || {
            let (lock, wake) = &*thread_stop;
            let mut stopped = match lock.lock() { Ok(value) => value, Err(_) => return };
            loop {
                let waited = match wake.wait_timeout(stopped, interval) { Ok(value) => value, Err(_) => return };
                stopped = waited.0;
                if *stopped { return; }
                drop(stopped);
                ...
                for attempt in 0..LEASE_RENEWAL_ATTEMPTS {
                    match store.renew_lease(item_id) {
                        ...
                        Err(error) => {
                            ...
                            stopped = match lock.lock() { Ok(value) => value, Err(_) => return };
                            let waited = match wake.wait_timeout(stopped, retry_delay) { Ok(value) => value, Err(_) => return };
                            stopped = waited.0;
                            if *stopped { return; }
                            drop(stopped);
                        }
        ...
        }).map_err(|_| PipelineError::new("STATE_CONFLICT", "lease renewal thread could not start"))?;
        Ok(Self { stop, failure, join: Some(join) })
    }

    pub fn check(&self) -> PipelineResult<()> {
        match self.failure.lock() {
            Ok(failure) => failure.clone().map_or(Ok(()), Err),
            Err(_) => Err(PipelineError::new("STATE_CONFLICT", "lease renewal state is unavailable")),
        }
    }
    ...
    fn stop_thread(&mut self) -> PipelineResult<()> {
        ...
        let mut stopped = lock.lock().map_err(|_| PipelineError::new(
            "STATE_CONFLICT", "lease renewal stop state is unavailable",
        ))?;
        *stopped = true;
        wake.notify_all();
        drop(stopped);
        if self.join.take().is_some_and(|join| join.join().is_err()) {
            return Err(PipelineError::new("STATE_CONFLICT", "lease renewal thread did not terminate cleanly"));
        }
        Ok(())
    }
}
...
impl Drop for LeaseKeeper {
    fn drop(&mut self) { let _ = self.stop_thread(); }
}
...
pub struct PipelineError { pub code: String, pub message: String }
...
impl WorkerFailure {
    pub fn new(code: impl Into<String>, retryable: bool, crashed: bool) -> Self {
        Self { code: code.into(), retryable, crashed, canceled: false }
    }
    pub fn crashed() -> Self { Self::new("WORKER_CRASHED", true, true) }
    pub fn canceled() -> Self {
        Self { code: "CANCELED".into(), retryable: false, crashed: false, canceled: true }
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ModelFailure { pub code: String, pub retryable: bool }

impl ModelFailure {
    pub fn retryable(code: impl Into<String>) -> Self { Self { code: code.into(), retryable: true } }
    pub fn fatal(code: impl Into<String>) -> Self { Self { code: code.into(), retryable: false } }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ParsedDocument { pub extracted: ExtractedDocument, pub image: Option<ImageInput> }
...
pub trait WorkerBoundary: Send + Sync {
    fn parse(
        &self,
        request_id: &str,
        path: &Path,
    ...
    fn shutdown(&self) -> Result<(), WorkerFailure> { Ok(()) }
}
...
pub trait ModelBoundary: Send + Sync {
    ...
    fn shutdown(&self) -> Result<(), ModelFailure> { Ok(()) }
}

impl ModelBoundary for ModelClient {
    fn propose(&self, document: &DocumentInput) -> Result<ModelProposal, ModelFailure> {
        ModelClient::propose(self, document).map_err(|error| ModelFailure::fatal(error.code().as_str()))
    }
}
...
pub struct CoreFileActions { store: Arc<QueueStore>, applier: FileApplier }
...
impl FileActions for CoreFileActions {
    ...
    fn apply(&self, item: &QueueItem, destination: &Path) -> PipelineResult<()> {
        ...
        let result = self.applier.apply(item.id, &item.source_path, destination, &item.source_hash);
        lease.stop_and_check()?;
        match result {
            Ok(receipt) => { self.store.complete_apply(item.id, receipt.id)?; Ok(()) }
            Err(error) => {
                if error.receipt().is_none() { let _ = self.applier.reconcile(item.id); }
                Err(error.into())
            }
        }
    }
    ...
    fn undo(&self, item: &QueueItem, receipt: &OperationReceipt) -> PipelineResult<()> {
        ...
        lease.stop_and_check()?;
        match result {
            Ok(undo_receipt) => { self.store.complete_undo(item.id, undo_receipt.id)?; Ok(()) }
            Err(error) => {
                if error.receipt().is_none() { let _ = self.applier.reconcile(item.id); }
                Err(error.into())
            }
    ...
    fn reconcile(&self, item: &QueueItem) -> PipelineResult<()> {
        self.applier.reconcile(item.id).map(|_| ()).map_err(Into::into)
    }
}
...
#[derive(Clone, Copy)]
enum LeasePhase { Extracting, Analyzing }
...
impl Pipeline {
    pub fn open(
        ...
        Ok(Self {
            ...
            paused: AtomicBool::new(false), active_item: AtomicI64::new(0),
            shutting_down: AtomicBool::new(false), model_timeout: std::time::Duration::from_secs(MODEL_TIMEOUT_SECONDS),
            lease_renewal_interval: LEASE_RENEWAL_INTERVAL, run_lock: Mutex::new(()),
        })
    }
    ...
    pub fn enqueue_files(&self, paths: &[PathBuf]) -> PipelineResult<Vec<QueueItem>> {
        ...
        if !queued.is_empty() { self.events.queue_changed(); }
        Ok(queued)
    }

    pub fn list(&self) -> PipelineResult<Vec<PipelineItem>> {
        self.store.list()?.into_iter().map(|item| {
    ...
    pub fn run_until_idle(&self) -> PipelineResult<()> {
        let _run = self.run_lock.lock().map_err(|_| PipelineError::new("STATE_CONFLICT", "pipeline lock is unavailable"))?;
    ...
    pub fn run_next(&self) -> PipelineResult<()> {
        let _run = self.run_lock.lock().map_err(|_| PipelineError::new("STATE_CONFLICT", "pipeline lock is unavailable"))?;
        if !self.paused.load(Ordering::SeqCst) { let _ = self.run_next_inner()?; }
        Ok(())
    }

    fn run_next_inner(&self) -> PipelineResult<bool> {
        let Some(item) = self.store.claim_next()? else { return Ok(false); };
        ...
        let lease = match LeaseKeeper::start_with_cancel(
            ...
            Arc::new(move || {
                match cancel_phase.lock().map(|phase| *phase) {
                    Ok(LeasePhase::Extracting) => { let _ = cancel_worker.cancel(&cancel_request_id); }
                    Ok(LeasePhase::Analyzing) => { let _ = cancel_model.cancel(); }
                    Err(_) => {
            ...
            Err(error) => {
                ...
                self.store.record_processing_failure(item.id, ErrorCode::IoError)?;
                self.events.queue_changed();
                return Err(error);
            }
        };
        self.events.queue_changed();
        let mut forward_progress = |mut progress: PipelineProgress| {
            progress.item_id = item.id;
            self.events.progress(progress);
        };
        let parsed = match self.worker.parse(&request_id, &item.source_path, &mut forward_progress) {
            Ok(parsed) => parsed,
            Err(error) => {
                self.active_item.store(0, Ordering::SeqCst);
                if self.shutting_down.load(Ordering::SeqCst) {
                    return Err(PipelineError::new("SHUTTING_DOWN", "pipeline is shutting down"));
                }
                ...
                if error.canceled { self.events.queue_changed(); return Ok(true); }
                let restart_failed = error.crashed && item.processing_failures == 0 && self.worker.restart().is_err();
                self.store.record_processing_failure(item.id, ErrorCode::IoError)?;
                if restart_failed {
        ...
        self.store.transition(item.id, QueueStatus::Extracting, QueueStatus::Analyzing, None)?;
        *phase.lock().map_err(|_| PipelineError::new("STATE_CONFLICT", "lease phase is unavailable"))? = LeasePhase::Analyzing;
        self.events.queue_changed();
        let packet = build_document_packet(parsed.extracted, parsed.image.is_some());
        let document = DocumentInput { text: packet.text.clone(), image: parsed.image };
        let proposal = match self.propose_with_deadline(&document) {
            ...
            Err(error) => {
                ...
                self.store.record_processing_failure(item.id, model_error_code(&error))?;
                if error.code == "MODEL_CANCEL_FAILED" { self.paused.store(true, Ordering::SeqCst); }
                self.events.queue_changed();
                return Ok(true);
            }
        };
        ...
        let extension = item.source_path.extension().and_then(|value| value.to_str()).unwrap_or_default();
        ...
        let filename = compose_filename(&outcome.proposal, extension, &existing_refs).value;
        let record = ProposalRecord {
            status: outcome.status,
            filename,
            description: outcome.proposal.description.clone(),
            reasons: outcome.reasons.iter().map(review_reason).map(str::to_owned).collect(),
            outcome,
            revision: 1,
        };
        let next = match record.status { ProposalStatus::Ready => QueueStatus::Ready, ProposalStatus::NeedsReview => QueueStatus::NeedsReview };
        lease.stop_and_check()?;
        self.repository.save_initial_and_transition(item.id, &record, next, self.store.session_id())?;
        let ready_item = self.store.list()?.into_iter().find(|candidate| candidate.id == item.id)
            .ok_or_else(|| PipelineError::new("ITEM_NOT_FOUND", "queue item disappeared after proposal storage"))?;
        self.active_item.store(0, Ordering::SeqCst);
        self.events.queue_changed();
    ...
    fn propose_with_deadline(&self, document: &DocumentInput) -> Result<ModelProposal, ModelFailure> {
        let model = Arc::clone(&self.model);
        let request_model = Arc::clone(&model);
        let document = document.clone();
        let (sender, receiver) = std::sync::mpsc::sync_channel(1);
        let join = std::thread::Builder::new().name("intern-model-request".into()).spawn(move || {
    ...
    fn apply_if_unchanged(&self, item: &QueueItem, filename: &str, settings: &AppSettings) -> PipelineResult<()> {
        ...
        let destination_root = if settings.destination.trim().is_empty() {
            item.source_path.parent().unwrap_or_else(|| Path::new(".")).to_path_buf()
        } else {
        ...
        if let Err(error) = self.files.apply(item, &destination_root.join(filename)) {
            ...
            if self.store.list()?.iter().any(|current| {
                current.id == item.id && current.status == QueueStatus::Ready
            }) {
                self.repository.mark_needs_review(item.id, &error.code)?;
            }
            self.events.queue_changed();
    ...
    pub fn pause(&self) { self.paused.store(true, Ordering::SeqCst); self.events.queue_changed(); }
    pub fn resume(&self) { self.paused.store(false, Ordering::SeqCst); self.events.queue_changed(); }
    ...
    pub fn cancel(&self, id: i64) -> PipelineResult<()> {
        let item = self.store.list()?.into_iter().find(|item| item.id == id)
            .ok_or_else(|| PipelineError::new("ITEM_NOT_FOUND", "queue item does not exist"))?;
        match item.status {
            QueueStatus::Queued | QueueStatus::Extracting | QueueStatus::Analyzing
            | QueueStatus::Ready | QueueStatus::NeedsReview => {
                self.store.transition(id, item.status, QueueStatus::Canceled, None)?;
            }
            QueueStatus::Canceled => {}
            QueueStatus::Applying => return Err(PipelineError::new("STATE_CONFLICT", "an atomic file operation cannot be canceled")),
            QueueStatus::Failed | QueueStatus::Completed => return Err(PipelineError::new("INVALID_TRANSITION", "item cannot be canceled")),
        }
    ...
    pub fn retry(&self, id: i64) -> PipelineResult<()> {
        let status = self.store.list()?.into_iter().find(|item| item.id == id)
            .ok_or_else(|| PipelineError::new("ITEM_NOT_FOUND", "queue item does not exist"))?.status;
        match status {
            QueueStatus::Failed => { self.store.manual_retry(id)?; }
            QueueStatus::Canceled => self.repository.retry_canceled(id)?,
            _ => return Err(PipelineError::new("INVALID_TRANSITION", "only failed or canceled items can be retried")),
        }
    ...
    pub fn approve(&self, id: i64, filename: &str, description: &str) -> PipelineResult<()> {
        ...
        let item = self.store.list()?.into_iter().find(|item| item.id == id)
            .ok_or_else(|| PipelineError::new("ITEM_NOT_FOUND", "queue item does not exist"))?;
        let source_extension = item.source_path.extension().and_then(|value| value.to_str()).unwrap_or_default();
        let approved_extension = Path::new(&filename).extension().and_then(|value| value.to_str()).unwrap_or_default();
        if source_extension.is_empty() || !source_extension.eq_ignore_ascii_case(approved_extension) {
            return Err(PipelineError::new("NAME_INVALID", "approved filename must preserve the source extension"));
        }
        if !matches!(item.status, QueueStatus::NeedsReview | QueueStatus::Ready) {
            return Err(PipelineError::new("INVALID_TRANSITION", "proposal is not reviewable"));
        }
        self.repository.approve_user_edit(id, item.status, &filename, description)?;
        let ready = self.store.list()?.into_iter().find(|candidate| candidate.id == id).unwrap();
        let settings = match self.settings.load() {
            Ok(settings) => settings,
            Err(error) => {
    ...
    pub fn undo(&self, id: i64) -> PipelineResult<()> {
        ...
        if item.status != QueueStatus::Completed {
            return Err(PipelineError::new("INVALID_TRANSITION", "only completed operations can be undone"));
        }
        let receipt = self.store.load_receipt(id)?.ok_or_else(|| PipelineError::new(
            "STATE_CONFLICT", "completed item has no durable operation receipt",
        ))?;
    ...
    pub fn recover(&self) -> PipelineResult<()> {
        let interrupted = self.store.list()?.into_iter().filter(|item| {
            matches!(item.status, QueueStatus::Extracting | QueueStatus::Analyzing)
        }).map(|item| item.id).collect::<Vec<_>>();
        self.store.recover_interrupted()?;
        for id in interrupted {
            if self.store.list()?.iter().any(|item| item.id == id && item.status == QueueStatus::Queued) {
                self.repository.record_recovered_failure(id)?;
            }
        }
        for item in self.store.list()?.into_iter().filter(|item| item.status == QueueStatus::Applying) {
...
struct PipelineRepository { connection: Mutex<Connection> }
...
impl PipelineRepository {
    fn open(path: &Path) -> PipelineResult<Self> {
        ...
        connection.execute_batch("PRAGMA foreign_keys=ON;").map_err(database_error)?;
        ...
        if legacy_exists {
            ...
            transaction.execute("DROP TABLE pipeline_proposals", []).map_err(database_error)?;
    ...
    fn save_initial_and_transition(
        ...
        let json = serde_json::to_string(record).map_err(|_| PipelineError::new("INVALID_DATA", "proposal could not be stored"))?;
        ...
        if changed != 1 { return Err(PipelineError::new("STATE_CONFLICT", "analyzing proposal transition compare-and-swap failed")); }
        transaction.execute(
    ...
    fn load_proposal(&self, id: i64) -> PipelineResult<Option<ProposalRecord>> {
        let json = self.lock()?.query_row(
            "SELECT proposal_json FROM proposals WHERE queue_item_id = ?1",
            params![id], |row| row.get::<_, String>(0),
        ).optional().map_err(database_error)?;
        json.map(|value| serde_json::from_str(&value).map_err(|_| PipelineError::new(
            "INVALID_DATA", "stored proposal is invalid",
        ))).transpose()
    }

    fn delete_proposal(&self, id: i64) -> PipelineResult<()> {
        self.lock()?.execute("DELETE FROM proposals WHERE queue_item_id = ?1", params![id]).map_err(database_error)?;
        Ok(())
    }

    fn mark_needs_review(&self, id: i64, reason: &str) -> PipelineResult<()> {
        let mut record = self.load_proposal(id)?.ok_or_else(|| PipelineError::new("INVALID_DATA", "proposal is missing"))?;
        record.status = ProposalStatus::NeedsReview;
        if !record.reasons.iter().any(|entry| entry == reason) { record.reasons.push(reason.to_owned()); }
        record.revision += 1;
        let mut connection = self.lock()?;
        let transaction = connection.transaction().map_err(database_error)?;
        let changed = transaction.execute(
        ...
        if changed != 1 { return Err(PipelineError::new("STATE_CONFLICT", "ready item changed before review")); }
        let json = serde_json::to_string(&record).map_err(|_| PipelineError::new("INVALID_DATA", "proposal could not be stored"))?;
    ...
    fn retry_canceled(&self, id: i64) -> PipelineResult<()> {
        ...
        if changed == 1 { Ok(()) } else { Err(PipelineError::new("STATE_CONFLICT", "canceled retry compare-and-swap failed")) }
    }
    ...
    fn record_recovered_failure(&self, id: i64) -> PipelineResult<()> {
        ...
        if changed == 1 { Ok(()) } else { Err(PipelineError::new("STATE_CONFLICT", "recovered item changed before failure accounting")) }
    }
    ...
    fn remove_item(&self, id: i64) -> PipelineResult<()> {
        ...
        if changed == 1 { Ok(()) } else { Err(PipelineError::new("STATE_CONFLICT", "active or missing item cannot be removed")) }
    }

    fn approve_user_edit(&self, id: i64, expected: QueueStatus, filename: &str, description: &str) -> PipelineResult<()> {
        let mut record = self.load_proposal(id)?.ok_or_else(|| PipelineError::new("INVALID_DATA", "proposal is missing"))?;
        ...
        if changed != 1 { return Err(PipelineError::new("STATE_CONFLICT", "review item changed before approval")); }
        let updated = transaction.execute(
            "UPDATE proposals SET proposal_json = ?1, created_at = unixepoch() WHERE queue_item_id = ?2",
            params![json, id],
        ).map_err(database_error)?;
        if updated != 1 { return Err(PipelineError::new("INVALID_DATA", "proposal is missing")); }
        transaction.commit().map_err(database_error)
    }

    fn lock(&self) -> PipelineResult<std::sync::MutexGuard<'_, Connection>> {
        self.connection.lock().map_err(|_| PipelineError::new("DATABASE_UNAVAILABLE", "pipeline database lock is unavailable"))
    }
}

fn model_error_code(error: &ModelFailure) -> ErrorCode {
    if error.code == "MODEL_RESPONSE_INVALID" { ErrorCode::ModelOutputInvalid } else { ErrorCode::IoError }
}
...
fn review_reason(reason: &ReviewReason) -> &'static str {
    match reason {
        ReviewReason::EvidenceMissing => "EVIDENCE_MISSING",
        ReviewReason::InvalidDate => "INVALID_DATE",
...
fn existing_names(directory: &Path) -> Vec<String> {
    fs::read_dir(directory).into_iter().flatten().filter_map(Result::ok)
