...
use crate::{
    model::{
        ...
        download::{CancellationToken, Downloader, ReqwestHttpTransport, SetupProgress, SystemDiskSpace},
...
pub struct FileSelectionDto { pub path: String, pub display_name: String }

#[derive(Clone, Debug, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct FolderSelectionDto { pub path: String, pub display_name: String }
...
impl From<PipelineError> for CommandError {
    fn from(error: PipelineError) -> Self { Self { code: error.code, message: error.message } }
}

impl From<crate::model::ModelError> for CommandError {
    fn from(error: crate::model::ModelError) -> Self {
        Self { code: error.code().as_str().into(), message: "local model operation failed".into() }
    }
}
...
#[derive(Clone, Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct EvidenceDto { date: Option<String>, r#type: Option<String>, parties: Option<String> }

#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(rename_all = "snake_case")]
enum SetupStatus { Ready, Required, Downloading, Failed }
...
struct TauriPipelineEvents { app: AppHandle }

impl PipelineEventSink for TauriPipelineEvents {
    fn queue_changed(&self) { let _ = self.app.emit("queue://changed", serde_json::json!({})); }
    fn progress(&self, progress: PipelineProgress) { let _ = self.app.emit("queue://progress", progress); }
}
...
impl RuntimeModel {
    fn new(executable: PathBuf, model_directory: PathBuf) -> Self {
        Self { executable, model_directory, client: RwLock::new(None), server: Mutex::new(None) }
    }

    fn installed(&self, manifest: &ModelManifest) -> bool {
        manifest.files.iter().all(|file| {
            crate::model::download::validate_selected_file(&self.model_directory.join(&file.name), file).is_ok()
        })
    }

    fn start(&self, manifest: &ModelManifest) -> Result<(), CommandError> {
        if !self.installed(manifest) {
            return Err(CommandError { code: "MODEL_NOT_READY".into(), message: "model files are not installed".into() });
        }
        ...
        *self.client.write().map_err(|_| CommandError { code: "MODEL_NOT_READY".into(), message: "model state is unavailable".into() })? = Some(client);
        *self.server.lock().map_err(|_| CommandError { code: "MODEL_NOT_READY".into(), message: "model process state is unavailable".into() })? = Some(server);
        Ok(())
    }

    fn stop_runtime(&self) -> Result<(), ModelFailure> {
        let stop_result = {
            let mut server = self.server.lock().map_err(|_| ModelFailure::fatal("MODEL_CANCEL_FAILED"))?;
            server.take().map(|mut server| {
                server.stop().map_err(|_| ModelFailure::fatal("MODEL_CANCEL_FAILED"))
            }).unwrap_or(Ok(()))
        };
        *self.client.write().map_err(|_| ModelFailure::fatal("MODEL_CANCEL_FAILED"))? = None;
        stop_result
    }
}
...
impl ModelBoundary for RuntimeModel {
    fn propose(&self, document: &DocumentInput) -> Result<ModelProposal, ModelFailure> {
        let client = self.client.read().map_err(|_| ModelFailure::fatal("MODEL_NOT_READY"))?;
        let client = client.as_ref().ok_or_else(|| ModelFailure::fatal("MODEL_NOT_READY"))?;
        client.propose(document).map_err(|error| ModelFailure::fatal(error.code().as_str()))
    }

    fn cancel(&self) -> Result<(), ModelFailure> {
        self.stop_runtime()?;
        let manifest = ModelManifest::embedded().map_err(|error| ModelFailure::fatal(error.code().as_str()))?;
        self.start(&manifest).map_err(|error| ModelFailure::fatal(error.code))
    }

    fn shutdown(&self) -> Result<(), ModelFailure> { self.stop_runtime() }
}
...
impl SetupManager {
    fn new(app: AppHandle, runtime: Arc<RuntimeModel>, manifest: &ModelManifest) -> Self {
        ...
        let state = SetupStateDto {
            state: if installed { SetupStatus::Ready } else { SetupStatus::Required },
            downloaded_bytes: if installed { total_bytes } else { 0 },
            total_bytes,
            error: None,
        };
        Self {
            app, runtime, state: Mutex::new(state), running: AtomicBool::new(false),
            scheduler: Mutex::new(None), model_ready: Arc::new(AtomicBool::new(installed)),
        }
    }

    fn get(&self) -> Result<SetupStateDto, CommandError> {
        self.state.lock().map(|state| state.clone()).map_err(|_| CommandError {
            code: "SETUP_UNAVAILABLE".into(), message: "setup state is unavailable".into(),
        })
    }
    ...
    fn start(self: &Arc<Self>) -> Result<(), CommandError> {
        ...
        std::thread::Builder::new().name("intern-model-setup".into()).spawn(move || {
            ...
            CommandError { code: "SETUP_UNAVAILABLE".into(), message: "setup thread could not start".into() }
        })?;
        Ok(())
    }
    ...
    fn download_and_start(&self) -> Result<u64, CommandError> {
        ...
        for file in &manifest.files {
            ...
            downloader.download(file, &self.runtime.model_directory, &cancellation, |progress: SetupProgress| {
                self.set_state(SetupStatus::Downloading, offset + progress.completed_bytes, None);
            })?;
    ...
    fn set_state(&self, state: SetupStatus, downloaded_bytes: u64, error: Option<String>) {
        ...
        if ready {
            if let Ok(scheduler) = self.scheduler.lock() {
                if let Some(sender) = scheduler.as_ref() { let _ = sender.send(SchedulerMessage::Wake); }
            }
        }
    }
}

enum SchedulerMessage { Wake, Shutdown }
...
impl PipelineScheduler {
    fn start(
        ...
        let join = std::thread::Builder::new().name("intern-pipeline-scheduler".into()).spawn(move || {
            loop {
                ...
                let (recover, drain) = scheduler_actions(timed_out, model_ready.load(Ordering::SeqCst));
                if recover {
                    if let Err(error) = scheduled_pipeline.recover() {
                        let _ = app.emit("queue://changed", serde_json::json!({ "error": error.code }));
                    }
                }
                if drain {
            ...
            code: "STATE_CONFLICT".into(), message: "pipeline scheduler could not start".into(),
        })?;
        Ok(Self { sender, join: Mutex::new(Some(join)), pipeline })
    }

    fn wake(&self) -> Result<(), CommandError> {
        self.sender.send(SchedulerMessage::Wake).map_err(|_| CommandError {
            code: "STATE_CONFLICT".into(), message: "pipeline scheduler is unavailable".into(),
        })
    }
}
...
impl Drop for PipelineScheduler {
    fn drop(&mut self) {
        ...
        if let Ok(join) = self.join.get_mut() {
            if let Some(join) = join.take() { let _ = join.join(); }
        }
    }
}
...
impl AppState {
    pub fn initialize(app: &AppHandle) -> Result<Self, CommandError> {
        let data = app.path().app_local_data_dir().map_err(|_| CommandError {
            code: "APP_DATA_UNAVAILABLE".into(), message: "local application data directory is unavailable".into(),
        })?;
        std::fs::create_dir_all(&data).map_err(|_| CommandError {
            code: "APP_DATA_UNAVAILABLE".into(), message: "local application data directory could not be created".into(),
        })?;
        let executable_directory = std::env::current_exe().ok().and_then(|path| path.parent().map(Path::to_path_buf))
            .ok_or_else(|| CommandError { code: "SIDECAR_UNAVAILABLE".into(), message: "application executable directory is unavailable".into() })?;
        let worker_name = if cfg!(windows) { "intern-worker.exe" } else { "intern-worker" };
        let server_name = if cfg!(windows) { "llama-server.exe" } else { "llama-server" };
        let model_directory = data.join("models");
        let runtime = Arc::new(RuntimeModel::new(executable_directory.join(server_name), model_directory));
        let manifest = ModelManifest::embedded()?;
        let setup = Arc::new(SetupManager::new(app.clone(), Arc::clone(&runtime), &manifest));
        ...
        let pipeline = Arc::new(Pipeline::with_local_files(
            ...
            Arc::new(SupervisedWorker::new(executable_directory.join(worker_name))),
            runtime,
        ...
        let scheduler = PipelineScheduler::start(
            Arc::clone(&pipeline), Arc::clone(&setup.model_ready), app.clone(),
        )?;
        *setup.scheduler.lock().map_err(|_| CommandError { code: "STATE_CONFLICT".into(), message: "setup scheduler state is unavailable".into() })? = Some(scheduler.sender.clone());
        let state = Self { pipeline, settings, setup, scheduler, app: app.clone() };
        if matches!(state.setup.get()?.state, SetupStatus::Ready) { state.schedule()?; }
        Ok(state)
    }

    fn schedule(&self) -> Result<(), CommandError> {
        if self.setup.model_ready.load(Ordering::SeqCst) { self.scheduler.wake()?; }
        Ok(())
    }
}
...
pub fn queue_list(state: State<'_, AppState>) -> Result<Vec<QueueItemDto>, CommandError> {
    state.pipeline.list()?.into_iter().map(queue_item_dto).collect()
}

#[tauri::command]
pub fn queue_add_files(files: Vec<FileSelectionDto>, state: State<'_, AppState>) -> Result<(), CommandError> {
...
#[tauri::command]
pub fn queue_add_folder(folder: FolderSelectionDto, state: State<'_, AppState>) -> Result<(), CommandError> {
...
pub fn queue_pause(state: State<'_, AppState>) -> Result<(), CommandError> {
    ...
    let _ = state.app.emit("queue://changed", serde_json::json!({ "paused": true }));
    Ok(())
}
...
pub fn queue_resume(state: State<'_, AppState>) -> Result<(), CommandError> {
    ...
    let _ = state.app.emit("queue://changed", serde_json::json!({ "paused": false }));
    state.schedule()
}
...
#[tauri::command]
pub fn proposal_approve(id: String, filename: String, description: String, state: State<'_, AppState>) -> Result<(), CommandError> {
    state.pipeline.approve(parse_item_id(&id)?, &filename, &description)?;
    Ok(())
}
...
#[tauri::command]
pub fn settings_save(mut settings: AppSettings, state: State<'_, AppState>) -> Result<(), CommandError> {
    if !settings.destination.trim().is_empty() {
        settings.destination = canonical_folder(Path::new(&settings.destination))?.to_string_lossy().into_owned();
    }
...
#[tauri::command]
pub fn setup_get(state: State<'_, AppState>) -> Result<SetupStateDto, CommandError> { state.setup.get() }

#[tauri::command]
pub fn setup_start(state: State<'_, AppState>) -> Result<(), CommandError> { state.setup.start() }
...
fn queue_item_dto(item: PipelineItem) -> Result<QueueItemDto, CommandError> {
    ...
    let evidence = proposal.map(|record| EvidenceDto {
        ...
        parties: (!record.outcome.proposal.parties.is_empty()).then(|| record.outcome.proposal.parties.join("; ")),
    });
    Ok(QueueItemDto {
        id: item.id.to_string(),
        original_filename: item.source_path.file_name().and_then(|name| name.to_str()).unwrap_or("Document").to_owned(),
        status: item.status,
        ...
        evidence,
        reason: proposal.filter(|record| !record.reasons.is_empty()).map(|record| record.reasons.join(", ")),
        error_code: item.error_code.map(|code| code.as_str().to_owned()),
        undoable: item.status == QueueStatus::Completed && item.receipt.as_ref().is_some_and(|receipt| {
